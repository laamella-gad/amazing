...
import static com.laamella.daedalousy.mazemodel.orthogonal.Grid.GridUtilityWrapper.*;
import static com.laamella.daedalousy.mazemodel.orthogonal.Square.Direction.*;
...
public class BinaryTreeMazeGenerator {
	...
	public void createMaze() {
		...
		final int width = grid.getWidthInSquares();
		final int height = grid.getHeightInSquares();

		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				final Square square = grid.getSquare(x, y);
				if (grid.isBorderSquare(WEST, x, y)) {
					if (!grid.isBorderSquare(NORTH, x, y)) {
						// Open the whole left column at the top
						square.getWall(NORTH).open();
					} else {
						// Create upper left entrance
						square.getWall(WEST).open();
					}
					continue;
				}
				if (grid.isBorderSquare(NORTH, x, y)) {
