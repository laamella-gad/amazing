 ...
 * current cell, remove the current cell from the list. The Maze is done when the
 * list becomes empty. The interesting part that allows many possible textures is
 * how you pick a cell from the list. For example, if you always pick the most
 * recent cell added to it, this algorithm turns into the recursive backtracker.
 * If you always pick cells at random, this will behave similarly but not exactly
 * to Prim's algorithm. If you always pick the oldest cells added to the list,
 * this will create Mazes with about as low a "river" factor as possible, even
 * lower than Prim's algorithm. If you usually pick the most recent cell, but
